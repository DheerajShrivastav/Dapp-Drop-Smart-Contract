...
> Pick-up list. v0.3 escrow + Merkle settlement, Phase 2 signature verification, and the invariant-test hardening pass (incl. a real security fix) are all merged to `dev` (PR #1, #2, #3). `cancelCampaign` is in progress on `feature/cancel-campaign` (84 tests passing). Ordered roughly by priority.
...
- [x] **`cancelCampaign` + refund.** In progress on `feature/cancel-campaign`. Allowed only in Draft/Open **and** while `campaign.totalParticipants == 0` (founder-flagged abuse concern: without the zero-participants gate, a host could let people do free work then cancel right before `Ended` to dodge paying out — closed by locking in the moment anyone genuinely engages). ERC20 refunded immediately, no grace period (safe since no root/claim was ever possible pre-Ended). NFTs reclaimed via the existing `withdrawUnclaimedERC721`/`withdrawUnclaimedERC1155`, now immediately callable (no grace wait) once `Cancelled`. New terminal `CampaignStatus.Cancelled`. 84 tests passing; not yet merged to `dev`.
