...
`dev` has v0.3 escrow + Merkle settlement (PR #1), Phase 2 signature verification (PR #2), the invariant-test hardening pass including a real security fix (PR #3), `cancelCampaign` (PR #4), on-chain tiered settlement + per-campaign module pinning (PR #6), its invariant suite (PR #7), the Merkle root dispute window (PR #8), that window's own invariant suite (PR #9), the protocol fee module (PR #10), the NFT settlement module extraction (PR #11), fee-on-transfer ERC20 support (PR #12), and its invariant coverage (PR #13) all merged. VERSION `0.5.0` on `dev`.

Active development on branch **`feature/hardening-sweep`**: three small backlog items bundled — `withdrawETH` restricted to a stored admin-settable treasury (clears Slither's `arbitrary-send-eth` finding), `.code.length` guards rejecting EOA/empty reward-token addresses at the configuration boundary, and multi-leaf NFT Merkle proof test coverage. See [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md).
...
Toolchain: Foundry 1.7.1, OZ + forge-std submodules initialized. **166 tests passing** (15 suites, including 6 stateful-fuzz invariant suites) on `feature/hardening-sweep`. Build clean, `forge fmt --check` clean. Contract size 23,499B runtime (24,576B limit — **~1,077B headroom**, down from ~1,318B; the treasury setter/getter/event/errors + `.code.length` guard cost ~241B).
...
- [BRANCHES.md](BRANCHES.md) — branch topology, merged branches, active hardening-sweep branch.
