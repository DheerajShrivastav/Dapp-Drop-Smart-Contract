...
`dev` has v0.3 escrow + Merkle settlement (PR #1), Phase 2 signature verification (PR #2), the invariant-test hardening pass including a real security fix (PR #3), `cancelCampaign` (PR #4), on-chain tiered settlement + per-campaign module pinning (PR #6), its invariant suite (PR #7), and the Merkle root dispute window (PR #8) all merged. VERSION `0.5.0` on `dev`.

Active development, in parallel, on two branches: **`feature/root-dispute-window-invariant`** (a stateful-fuzz invariant suite for `ROOT_DISPUTE_WINDOW`) and **`feature/protocol-fee-module`** (a `FeeModule` satellite contract, mirroring `OnChainRewardModule`, skimming a flat fee at `fundCampaignERC20` — see [ARCHITECTURE.md](ARCHITECTURE.md)).
...
Toolchain: Foundry 1.7.1, OZ + forge-std submodules initialized. **141 tests passing** (12 suites, including 4 stateful-fuzz invariant suites) on `feature/protocol-fee-module`. Build clean, `forge fmt --check` clean. Contract size 23,780B runtime (24,576B limit — **~796B headroom, critical**; the fee module itself proved the satellite-contract pattern is now mandatory for any feature with real logic, but even minimal wiring still costs several hundred bytes — 1-2 more integrations this size and the entrypoint is out of room).
...
- [BRANCHES.md](BRANCHES.md) — branch topology, merged branches, active root-dispute-window branch.
