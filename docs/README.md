...
`dev` has v0.3 escrow + Merkle settlement (PR #1), Phase 2 signature verification (PR #2), the invariant-test hardening pass including a real security fix (PR #3), `cancelCampaign` (PR #4), on-chain tiered settlement + per-campaign module pinning (PR #6), its invariant suite (PR #7), the Merkle root dispute window (PR #8), that window's own invariant suite (PR #9), the protocol fee module (PR #10), the NFT settlement module extraction (PR #11), and fee-on-transfer ERC20 support (PR #12) all merged. VERSION `0.5.0` on `dev`.

Active development on branch **`feature/fee-on-transfer-invariant`**: a 6th stateful-fuzz suite, `FeeOnTransferEscrowSolvency.invariant.t.sol`, fuzzing the same escrow-solvency properties as `EscrowSolvency.invariant.t.sol` against a fee-on-transfer token — a follow-up flagged (non-blocking) during PR #12's review, since the existing invariant handler only ever wired up a standard `ERC20Mock`. See [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md).
...
Toolchain: Foundry 1.7.1, OZ + forge-std submodules initialized. **156 tests passing** (15 suites, including 6 stateful-fuzz invariant suites) on `feature/fee-on-transfer-invariant`. Build clean, `forge fmt --check` clean. Contract size unchanged from PR #12 (23,258B runtime, ~1,318B headroom against the 24,576B limit) — this pass is test-only.
...
- [BRANCHES.md](BRANCHES.md) — branch topology, merged branches, active fee-on-transfer-invariant branch.
