...
- `SETTLER_ROLE` — platform-controlled keys authorized to fill a Merkle-settlement vacuum on an abandoned campaign, strictly as a last-resort fallback: `setERC20MerkleRoot`, `NFTSettlementModule.setNFTMerkleRoot`, and `closeCampaign` all accept a `SETTLER_ROLE` caller once `SETTLEMENT_FALLBACK_DELAY` (14 days) has elapsed since `endTime`, but only to publish a root that has never been published (can never override/correct/race a host-published one) or to close a campaign that already has one. The host retains full authority at all times, including after a settler acts. See [REWARD_SYSTEM.md](REWARD_SYSTEM.md) and [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md).
...
- `closeCampaign` (**host-only for a normal campaign; `SETTLER_ROLE` may close as a last-resort fallback**): Ended→Closed (records `_campaignClosedAt` to start the unclaimed-sweep grace window). Kept host-gated for the normal path because closing freezes the Merkle root, so an open-permissionless close would let a griefer lock the host out of finalizing/correcting allocations. The settler-fallback branch exists for a genuinely abandoned campaign whose host never returns: `SETTLER_ROLE` may close only once status is Ended, a Merkle root exists on either path (host- or settler-published — never a settler closing with nothing ever claimable), and `SETTLEMENT_FALLBACK_DELAY` has elapsed since `endTime`. Sweep functions stay host-only and pay only the host either way. See [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md).
- `cancelCampaign` (host): Draft, Open, **or Ended** → Cancelled, **only while `campaign.totalParticipants == 0` AND no settlement has been committed on either the ERC20 or NFT path** (`_erc20SettlementMode == UNSET` and no NFT root published on the pinned `NFTSettlementModule`). Both gates matter once `Ended` is in scope: `totalParticipants` alone is NOT sufficient, since it only tracks `completeTask` engagement and is fully decoupled from Merkle settlement — an allowlist-based campaign can have real, partially-claimed rewards while `totalParticipants` stays 0 forever, so the settlement-commitment check exists specifically to stop a host from cancelling out from under a published root to instantly reclaim funds already allocated to (but not yet claimed by) participants. The `totalParticipants` guard separately closes a bait-and-switch griefing path where a host could let participants do free work and cancel right before Ended to dodge paying out. `Ended` is included in the status check specifically because `endCampaign` is permissionless: a keeper can end a past-deadline campaign that never had a single participant or any settlement committed, and the host must still get the same immediate refund rather than being forced into the 30-day `closeCampaign` grace path. `Closed` is deliberately excluded — once closed, the host has already chosen the grace-period path. Refunds escrowed ERC20 immediately (no grace period — safe because the settlement-commitment guard means no claim was ever possible). Escrowed NFTs are reclaimed via the existing `withdrawUnclaimedERC721`/`withdrawUnclaimedERC1155`, which become immediately callable (no grace wait) once a campaign is `Cancelled` — the NFT settlement-commitment check only blocks cancelling a campaign with an already-published NFT root, it does not change this reclaim path. See [REWARD_SYSTEM.md](REWARD_SYSTEM.md).

## Reward / claim flow (v0.3 — escrow + Merkle settlement)

ERC20 (Stage B1, done): `configureERC20Reward` (Draft) → `fundCampaignERC20` (escrow into contract, crediting the amount **actually received** rather than the nominal amount requested — fee-on-transfer-safe, see [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md)) → run campaign → `endCampaign` → `setERC20MerkleRoot` (off-chain allocations, host **or**, as a last-resort fallback for an abandoned campaign, `SETTLER_ROLE` — see below; starts a `ROOT_DISPUTE_WINDOW`, currently 24h) → participants `claimERC20(amount, proof)` from escrow once the window elapses (reverts `RootDisputeWindowActive` before then, `AlreadySwept` after host sweep — required because ERC20 escrow is a commingled pool, see [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md) #3) → after `Closed` + 30-day grace, host `withdrawUnclaimedERC20`. See [REWARD_SYSTEM.md](REWARD_SYSTEM.md).

NFT (Stage B2, done): `depositERC721Rewards`/`depositERC1155Rewards` (escrow per campaign, `Web3Campaigns` custodies via OZ `ERC721Holder`/`ERC1155Holder`) → `endCampaign` → `setNFTMerkleRoot` (host or settler fallback, identical rule to the ERC20 path; also starts the dispute window) → participants `claimNFT(standard, token, tokenId, amount, proof)` once it elapses → host `withdrawUnclaimedERC721`/`withdrawUnclaimedERC1155` after grace. `setNFTMerkleRoot`/`claimNFT`/`withdrawUnclaimedERC721`/`withdrawUnclaimedERC1155` are called on the campaign's `NFTSettlementModule`, not on `Web3Campaigns` directly — see below.

**Settlement-liveness fallback (`SETTLER_ROLE`):** permissionless `endCampaign` guarantees a campaign reaches `Ended`, but a Merkle campaign still needed *someone* to publish a root — if a host abandoned a campaign after participants engaged, no root would ever come, nobody could claim, the host couldn't `cancelCampaign` (blocked by `totalParticipants != 0`), and escrow would strand forever. `SETTLER_ROLE` (platform-controlled, admin-granted like `SIGNER_ROLE`) closes that gap: once a campaign has been `Ended` for `SETTLEMENT_FALLBACK_DELAY` (14 days) with no root ever published, a settler may publish one — but ONLY into a vacuum: it can never override, correct, or race a root the host already set, and the host retains full authority at all times, including publishing/correcting a root *after* a settler has acted (identical dispute-window rearm behavior either way). See [SECURITY_FINDINGS.md](SECURITY_FINDINGS.md) for the full trust-model note.
