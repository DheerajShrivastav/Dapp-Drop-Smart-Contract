...
**Active work (two parallel branches, both off `dev` post-PR #8):** `feature/root-dispute-window-invariant` — a stateful-fuzz invariant suite for `ROOT_DISPUTE_WINDOW` (123 tests). `feature/protocol-fee-module` — a `FeeModule` satellite contract (mirrors `OnChainRewardModule`; no per-campaign pinning needed, since fee computation has no persistent per-campaign state) skimming a flat fee at `fundCampaignERC20` (141 tests). Neither merged yet.
